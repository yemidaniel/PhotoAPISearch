...
let current_page;
let next_page;
function loadMoreData(next_url, page_number) {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4) {
                if (this.status == 200) {
                    ...
                    const numOfPhotoDivs = photoDivs.length;
                    let photoDivHTMLArr = [];
                    for (let i = 0; i < numOfPhotoDivs; i++) {
                        photoDivHTMLArr[i] = '';
                    }
                    let photosData = JSON.parse(xhttp.responseText);
